...
public:
  ...
  int cqi_info(uint32_t tti, uint16_t rnti, uint32_t enb_cc_idx, uint32_t cqi_value, uint32_t transmission_mode) override;
...
private:
  ...
  // SCOPE: dictionary to keep track of last time DL SINR was written on file
  std::map<uint16_t, long int> rnti_metric_timestamp;
