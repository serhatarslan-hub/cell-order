...
private:
  ...
  public:
    explicit ue(uint16_t rnti_, bool pcell_) : rnti(rnti_), pcell(pcell_)
    {
      ...
      tx_data_pow_avg = 0;
      tx_data_pow_samples = 0;
      last_time_mcs_saved = 0;
    }

    bool                 is_grant_available = false;
    srslte_phich_grant_t phich_grant        = {};

    // SCOPE: variables to measure average transmission power to this user
    double tx_data_pow_avg;
    int tx_data_pow_samples;
    double channel_coeff;

    // SCOPE: when was mcs last saved
    long int last_time_mcs_saved;
    ...
    void set_power_multiplier(srslte_dl_cfg_t* dl_cfg, float rho_a) {
        dl_cfg->pdsch.p_a = srslte_convert_amplitude_to_dB(rho_a);
    }
