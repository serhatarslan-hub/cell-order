...
sched_ue::sched_ue() : log_h(srslte::logmap::get("MAC "))
{
  ...
  last_time_prb_logged_ms = 0;
  slice_number = -1;
  imsi = 0;
  has_slicing_mask = 0;
  slicing_mask = rbgmask_t(cell_rbgs);
}
...
// SCOPE: get uplink MCS
int sched_ue::get_ul_fixed_mcs(uint32_t cc_idx) {

    auto p   = get_cell_index(cc_idx);
    if (p.first) {
        return carriers[p.second].fixed_mcs_ul;
    }

    return -1;
}

// SCOPE: get downlink MCS
int sched_ue::get_dl_fixed_mcs(uint32_t cc_idx){

    auto p   = get_cell_index(cc_idx);
    if (p.first) {
        return carriers[p.second].fixed_mcs_dl;
    }

    return -1;
}

// SCOPE: fix user downlink modulation
void sched_ue::set_fixed_mcs_dl(int mcs_dl, uint32_t cc_idx) {

    std::lock_guard<std::mutex> lock(std::mutex);

    auto p   = get_cell_index(cc_idx);
    if (p.first) {
        carriers[p.second].fixed_mcs_dl = mcs_dl;
    }
}
...
// SCOPE: function to get cell PRBs
uint32_t sched_ue::get_serving_cell_prbs() {
    return cell.nof_prb;
}
...
// SCOPE: pass extra parameter to the function
                                     ...
                                     sched_ue*                         user)
{
  ...
  // SCOPE: scheduling configuration file
  std::string sched_file_name = SCOPE_CONFIG_DIR;
  sched_file_name += "config/ue_config_scheduling.txt";

  // SCOPE: scheduling threshold before acting on MAC scheduling
  int sched_parameter_ue = -1;

  // SCOPE: check if RNTI is that of a user
  bool rnti_is_user = is_user(user->get_rnti());

  // SCOPE: increase counter
  if (user->times_scheduled <= sched_threshold)
      user->times_scheduled += 1;
  else if (user->times_scheduled == sched_threshold + 1) {
      user->times_scheduled += 1;
  }

  switch (dci_format) {
    case SRSLTE_DCI_FORMAT1:
        // SCOPE: mute scheduling if past the threshold
        if (user->times_scheduled > sched_threshold) {
            // update past threshold state
            data->past_sched_threshold = true;

            if (rnti_is_user) {
                // Read user-specific scheduling parameter from configuration file
                sched_parameter_ue = read_ue_value_from_file(user->get_rnti(), sched_file_name);
            }

            if (sched_parameter_ue > -1)
                tbs = sched_parameter_ue;
            else
...
rbg_range_t sched_ue::get_required_dl_rbgs(uint32_t ue_cc_idx)
{
  ...
  int         pending_prbs =
      carriers[ue_cc_idx].get_required_prb_dl(req_bytes.first, cellparams->sched_cfg->max_nof_ctrl_symbols, false);
  if (pending_prbs < 0) {
  ...
  pending_prbs = carriers[ue_cc_idx].get_required_prb_dl(req_bytes.second, cellparams->sched_cfg->max_nof_ctrl_symbols, false);
...
// SCOPE: get number of RBGs from PRBs
rbg_range_t sched_ue::get_rbgs_from_prbs(uint32_t ue_cc_idx, int prb_num) {

  const auto* cellparams = carriers[ue_cc_idx].get_cell_cfg();

  if (prb_num < 0) {
    // printf("cellparams->nof_prb() %d\n", cellparams->nof_prb());
    return {0, 0};
    // return {cellparams->nof_prb(), cellparams->nof_prb()};
  }

  uint32_t rbg_num = cellparams->prb_to_rbg(prb_num);

  // set 1 as min RBG
  // TODO: check correctness
  // return {0, rbg_num};
  return {rbg_num, rbg_num};  // {rbg_min, rbg_max}
}
...
{
    uint32_t required_prb = 0;
    int req = carriers[cc_idx].get_required_prb_dl(req_bytes, nof_ctrl_symbols, true);
    
    if (req > 0) {
      required_prb = (uint32_t) req;
    }
...
// SCOPE: passing extra sc_stat for statistics purposes
int sched_ue_carrier::get_required_prb_dl(uint32_t req_bytes, uint32_t nof_ctrl_symbols, bool sc_stat)
{
  int      mcs    = 0;
  uint32_t nof_re = 0;
  int      tbs    = 0;
