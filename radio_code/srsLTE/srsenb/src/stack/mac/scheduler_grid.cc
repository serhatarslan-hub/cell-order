...
void sf_grid_t::init(const sched_cell_params_t& cell_params_)
{
  ...
  // SCOPE: save number of cell RBGs in global variable
  cell_rbgs = nof_rbgs;
                                        ...
                                        sched_interface::dl_sched_res_t*    dl_result)
{
  for (const auto& data_alloc : data_allocs) {
    ...
    // SCOPE: modify SRSLTE_DCI_FORMAT1 if necessary
    // pass extra parameter to the function
    int tbs = user->generate_dl_dci_format(
        data_alloc.pid, data, get_tti_tx_dl(), cell_index, tti_alloc.get_cfi(), data_alloc.user_mask, user);
